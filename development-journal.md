...
8.**MOST IMPORTANT**: The agent with gpt-5-nano keeps calling the tool, despite the tool limit middleware. This stops the agent from generating a message after a long time. The tool might've been called hundreds of times (or more).
- I used to think it was purely due to the latency of running LLM locally (which did not make sense in hindsight), so I waited for 3-7 minutes for a response every time, until one day it did not generate a message after 15min. I streamed all updates and found out the insane behavior of the agent, calling the tool hundreds of times. I finally thought of trying a different model. As soon as I switched to gpt-4o, the issue was gone. gpt-4o turned out to be much more robust and reliable in deciding tool usage, and much faster. Since then, my chatbot is at least "usable".
